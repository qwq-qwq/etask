...
//			$sales[$k]['Est_deliv_color'] = $est_deliv['color'];
	...
	function OnRecalcDiscount() {
		if ($this->order_data['Pay_state'] == 0 && $this->order_data['Ord_state'] < 50) {//Dont change anything if order already payed or sent
			...
			foreach ($order_goods as $article) {
				if (strtoupper($article['Discount_forbidden']) != 'YES') {
					if($all_discount){
				...
				$total_sum += round($article['Price']*(1 - $article['discount']/100)*$article['Qty'], 2);
			}
		...
		*/
		if ($paritetAfterChastichnoTask !== false) {
			...
			if (count($prevTask)) {
				...
				if ($res === FALSE){
					mail('jeck.solovyev@gmail.com', 'callcentre line '.__LINE__.' sql error', $sql);
					break;
				}
				...
				if ($number_inv === FALSE){
					mail('jeck.solovyev@gmail.com', 'callcentre line '.__LINE__.' sql error', 'ora_function MZ.GETNUMBERINVOICE');
					break;
				}
	...
	private function ChooseAssemblerShop($goods, $graph = array()) {
		...
		if($this->order_data['Delivery_type'] == 3) {//Self-delivery
			...
			if ($test['Qty'] == $count_goods) {//Shops set found. Choose among them
				...
				foreach ($shops as $shop) {
					...
					if(in_array($shop['Region'],array('Одесса', 'Днепропетровск', 'Винница'))) {
						/* не разрешать поиск в других магазинах, собирать только с Киева */
						//$tryit[] = $shop['Shop_id'];
					}
				}
			...
			if ($test['Qty'] == $count_goods) {//Shops set found. Choose among them
				...
				// не важно, что делалось выше мегапрограммистами делаем поставку с Киева
				foreach ($regions as $row) {
					if ($row['Region'] == 'Киев') {
						$region = $row['Region'];
						break;
...
//				if ($dep_id==PARITET_SHOP_ID) $dep_id = PARITET_REPLACEMENT_SHOP_ID;
	...
	private function noticeMe($str,$data=array()){
		...
		mail('developers@bukva.ua','build graph',$str.print_r($data,true));
	}
}