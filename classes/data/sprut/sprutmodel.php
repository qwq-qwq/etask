...
Class SprutModel extends OciDriver {
	...
	function getFirmByShop($code_shop){
	    if ($code_shop == 79 || $code_shop == 86 || $code_shop == 57 || $code_shop == 50 || $code_shop == 85){
		$code_firm = 6468101; 
	    }else{
		$code_firm = 6507142;
	    }
...
//		}
	...
	function GetStockGoods($goods = array(), $filter = array()) {
		...
		$SQL .= ' ORDER BY RANK_ESHOP DESC, QTY  DESC';
//		file_put_contents('/tmp/etasks_log.txt', "sql getstockgoods: ".$SQL."\n",FILE_APPEND);
		return $this->query($SQL);
	}
	...
	function CreateInvoice($ord, $goods, $from_shop_id, $to_shop_id, $change_state = true, $change_second_state = true){
		...
		$code_woi = $this->ora_function("mz.GetCodeWriteOffInvoice");
		file_put_contents('/opt/etasks/log.txt', "GetCodeWriteOffInvoice: ".$code_woi."\n",FILE_APPEND);
		if ($code_woi === FALSE){
			$this->rollback_tran();
			return -1;
		}
		$num_woi = $this->ora_function("mz.GetNUmberWriteOffInvoice");
		file_put_contents('/opt/etasks/log.txt', "GetNumberWriteOffInvoice: ".$num_woi."\n",FILE_APPEND);
		if ($num_woi === FALSE){
			$this->rollback_tran();
			return -2;
		}
		...
		file_put_contents('/opt/etasks/log.txt', "from_wh_code: ".$from_wh_code."\n",FILE_APPEND);
		$to_wh_code = $this->ora_function("MZ.GetTradeHallFromShop", array($to_shop_id));
		file_put_contents('/opt/etasks/log.txt', "to_wh_code: ".$to_wh_code."\n",FILE_APPEND);
		$to_dealer_code = $this->ora_function("MZ.GetCodeDealerFromShop", array($to_shop_id));
		file_put_contents('/opt/etasks/log.txt', "to_dealer_code: ".$to_dealer_code."\n",FILE_APPEND);
		$total_qty = 0;
		...
		.$this->getFirmByShop($from_shop_id).",'$from_wh_code','T','O','02','PD',
		'F', ".$this->getFirmByShop($to_shop_id).", '$to_wh_code', '$to_shop_id', '$comment','$to_dealer_code')";
		
		file_put_contents('/opt/etasks/log.txt', "sql: ".$sql."\n",FILE_APPEND);
		...
		foreach($goods as $article){
			...
			file_put_contents('/opt/etasks/log.txt', "result qty: ".$qty." art:".$article['intArticleID']." Qty:".$article['intDemandQty']."\n",FILE_APPEND);
		...
		if ($change_state) {
			...
			file_put_contents('/opt/etasks/log.txt', "res 1 state: ".$res."\n",FILE_APPEND); 
			...
			if ($change_second_state) {
				...
				file_put_contents('/opt/etasks/log.txt', "res 2 state: ".$res."\n",FILE_APPEND);
	...
	function CreateOrder($order, $goods){
		...
		foreach($goods as $article){
			...
			$price_no_vat = round(sprintf('%.3f', 100*$article['PriceDiscount']/(100 + $article['Vat'])),2);

			$result = $this->ora_function('MZ.SPR$_ORDER_CLIENT.InsertWaresOrder'
						, array(
									$ord_code
								  , $article['Wares_id']
								  , $article['Qty']
