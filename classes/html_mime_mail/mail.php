...
//				var_dump($images_dir . $images[1][$i]);
    ...
    function buildMessage($params = array())
    {
        ...
        switch (true) {
            ...
            case $html AND !$attachments AND $html_images:
            	...
            	$this->_addTextPart($message, $this->html_text);
            	$params['content_type'] = 'multipart/related';
	        	$related = &$message->addSubPart('', $params);
            ...
            case $html AND $attachments AND !$html_images:
                ...
                if (!is_null($this->html_text)) {
                	...
                	$alt = &$this->_addAlternativePart($null);
                    $this->_addTextPart($alt, $this->html_text);
                    $this->_addHtmlPart($alt);
                    
                    $message->_subparts[] = $alt;
                } else {
                    $this->_addHtmlPart($message);
                }
                for ($i=0; $i<count($this->attachments); $i++) {
...
//		die('itwas'.$this->itwas);
        if (isset($message)) {
            ...
            $server_url = parse_url(PROJECT_URL);
            $id_postfix = $GLOBALS['HTTP_SERVER_VARS']['HTTP_HOST'];
            if (empty($id_postfix)) $id_postfix = $server_url['host'];
            $message_id = sprintf('<%s.%s@%s>', base_convert(time(), 10, 36), base_convert(rand(), 10, 36), $id_postfix);
